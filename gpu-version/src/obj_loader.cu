  ...
  //merge the new information with previous information
    ...
    //save new information
      ...
      for(int i = 0 ; i < v_index.size() / 3 ; ++i) 
        _triangles.push_back(Triangle{emissive, 
                                      newPositionsStartIndex + Vec3i{v_index[3*i], v_index[3*i+1], v_index[3*i+2]},
                                      newNormalsStartIndex   + Vec3i{n_index[3*i], n_index[3*i+1], n_index[3*i+2]},
                                      newColorStartIndex     + Vec3i{0,0,0}});
    //save new information end
    return true;
}

const std::vector<Vec3f>& ObjLoader::getPositions() {
  return _positions;
}

const std::vector<Vec3f>& ObjLoader::getNormals() {
  return _normals; 
}

const std::vector<Vec3f>& ObjLoader::getColors() {
  return _colors;
}

const std::vector<Triangle>& ObjLoader::getTriangles() {
  return _triangles;
}